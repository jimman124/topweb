...
import com.mancom.security.AjaxAuthenticationFailureHandler;
import com.mancom.security.AjaxAuthenticationSuccessHandler;
import com.mancom.security.AjaxLogoutSuccessHandler;
import com.mancom.security.AuthoritiesConstants;
import com.mancom.security.CustomAccessDeniedHandler;
import com.mancom.security.Http401UnauthorizedEntryPoint;
import com.mancom.web.filter.CsrfCookieGeneratorFilter;
...
public class SecurityConfiguration extends WebSecurityConfigurerAdapter {
	...
	protected void configure(HttpSecurity http) throws Exception {
		http.csrf().ignoringAntMatchers("/websocket/**").and()
				...
				.deleteCookies("JSESSIONID", "CSRF-TOKEN").permitAll().and().headers().frameOptions().disable().and()
				.authorizeRequests().antMatchers("/api/register").permitAll().antMatchers("/api/activate").permitAll()
